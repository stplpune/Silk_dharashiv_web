...
export class AuthInterceptor implements HttpInterceptor {
  ...
  callDateTimeApi() {
    ...
    //     let expireAccessToken: any = (Math.round(new Date(localStorageData?.responseData1?.expireAccessToken).getTime() / 1000)); // logintokentime
    //     let tokenExpireDateTime: any = (Math.round(new Date(localStorageData?.responseData1?.refreshToken.expireAt).getTime() / 1000)); // loginexpiretokentime  
    //     if (this.currentDateTime >= expireAccessToken) {
    //       if (this.currentDateTime <= tokenExpireDateTime) { 
    //         let obj = {
    //           refreshToken: localStorageData?.responseData1?.refreshToken?.tokenString,
